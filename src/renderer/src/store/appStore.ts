...
export const useAppStore = create<AppState>((set, get) => ({
    ...
    updateRole: (id, updates) => set((state) => {
        ...
        const newActiveSessionId = activeSessionStillExists
            ...
            : (newSessions.length > 0 ? newSessions[0].id : 'neuro')
    ...
    toggleTerminalLayoutMode: () => set((state) => ({
    ...
    reorderTerminals: (sessionId, newOrder) => set((state) => {
        const session = state.sessions.find(s => s.id === sessionId)
        if (!session) return state
        ...
        const missingTerminals = session.terminals.filter(
        ...
        try {
            if (window.api?.claudeHistory) {
                ...
                if (project) {
                    ...
                    const deletedIds = await window.api.claudeHistory.getDeletedSessionsForProject(project.encodedPath)
                    ...
                    window.api.claudeHistory.copyToWorkspace(workspacePath, project.encodedPath).catch(err => {
    ...
    refreshClaudeHistory: async () => {
        ...
        try {
            if (window.api?.claudeHistory) {
                ...
                const deletedIds = await window.api.claudeHistory.getDeletedSessionsForProject(claudeProject.encodedPath)
                set({ deletedSessionIds: deletedIds })
                ...
                // Copy to workspace - 在后台进行，不阻塞 UI
                window.api.claudeHistory.copyToWorkspace(
                    workspacePath,
