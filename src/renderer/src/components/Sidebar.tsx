...
const ResizeIcon = () => (
  <svg viewBox="0 0 16 16" fill="currentColor" width="12px" height="12px">
    <path
      fillRule="evenodd"
      d="M2.5 13.5A.5.5 0 0 1 3 13h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zM2 2h12v12H2V2z"
    />
  </svg>
)
...
const calculateSessionMetrics = (stats: SessionTokenStats | null) => {
  ...
  const avgTokensPerMessage =
    stats.message_count > 0 ? Math.round(stats.total_tokens / stats.message_count) : 0
  const durationMs =
...
const UnifiedToolExecutionRenderer = ({
  toolUse,
  toolResults
}: {
  toolUse: ContentItem
...
const MessageRenderer = ({ message }: { message: any }) => {
  ...
  const hasContent = useMemo(() => {
    ...
    if (Array.isArray(content)) {
      const nonToolResultItems = content.filter((item: any) => {
        if (!item) return false
        if (item.type === 'tool_result') return false
        return true
      })
      if (nonToolResultItems.length === 0) return false
      const nonEmptyItems = nonToolResultItems.filter((item: any) => {
  ...
  const isSimpleText = useMemo(() => {
    ...
    if (Array.isArray(content))
      return content.every((item: any) => !item || item.type === 'text' || !item.type)
    return false
  }, [content])
...
const ConversationItem = ({ session, onSelect, onDelete }: ConversationItemProps) => {
  ...
  return (
    ...
    >
      <div className="ch-conversation-content" onClick={() => !isDeleting && onSelect(session)}>
        <div className="ch-conversation-preview">{session.preview || 'Empty conversation'}</div>
        <div className="ch-conversation-meta">
          <span className="ch-message-count">{session.messageCount} msgs</span>
      ...
      {isDeleting ? (
        <div className="ch-delete-confirm">
          <button className="ch-confirm-yes" onClick={handleDeleteClick} title="确认删除">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
          ...
          <button className="ch-confirm-no" onClick={handleCancelDelete} title="取消">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
...
const TokenDistributionChart = ({
  ...
  distribution: { input: number; output: number; cache_creation: number; cache_read: number }
  total: number
}) => {
  const items = [
      ...
      <div className="ch-metrics-grid">
        ...
        <div className="ch-metric-card">
          ...
          <div className="ch-metric-value">
            ...
            <span className="ch-metric-unit">m</span>
          </div>
          ...
          <div className="ch-timeline-start">
            ...
            <div className="ch-timeline-time">
              {stats.first_message_time
                ? new Date(stats.first_message_time).toLocaleTimeString('zh-CN', {
                    hour: '2-digit',
                    minute: '2-digit'
                  })
                : '-'}
          ...
          <div className="ch-timeline-end">
            ...
            <div className="ch-timeline-time">
              {stats.last_message_time
                ? new Date(stats.last_message_time).toLocaleTimeString('zh-CN', {
                    hour: '2-digit',
                    minute: '2-digit'
                  })
                : '-'}
      ...
      {stats.most_used_tools.length > 0 && (
        <div className="ch-section-card">
          <div className="ch-section-header">
            ...
            <span>工具使用</span>
            <span className="ch-tool-total">
              {stats.most_used_tools.reduce((sum, t) => sum + t.usage_count, 0)} 次
            </span>
          </div>
                ...
                return (
                  <div key={idx} className="ch-tool-usage-item">
                    ...
                    <div className="ch-tool-bar-container">
                      <div className="ch-tool-bar-fill" style={{ width: `${percentage}%` }} />
                    </div>
                  </div>
                )
              })
      ...
      <div className="ch-section-card">
        ...
        <div className="ch-token-details">
          ...
          <div className="ch-detail-row">
            ...
            <span className="ch-detail-value purple">
              {formatNumber(stats.total_output_tokens)}
            </span>
          </div>
          <div className="ch-detail-row">
            <span className="ch-detail-label">缓存创建</span>
            <span className="ch-detail-value blue">
              {formatNumber(stats.total_cache_creation_tokens)}
            </span>
          </div>
          <div className="ch-detail-row">
            <span className="ch-detail-label">缓存读取</span>
            <span className="ch-detail-value amber">
              {formatNumber(stats.total_cache_read_tokens)}
            </span>
          </div>
        ...
        <div className="ch-session-info">
          ...
          <div className="ch-info-row">
            ...
            <span className="ch-info-value">
              {formatNumber(metrics.avgTokensPerMessage)} tokens
            </span>
          </div>
        </div>
      </div>
...
const ConversationSlidePanel = ({ session, onClose }: ConversationSlidePanelProps) => {
  ...
  const [viewMode, setViewMode] = useState<'messages' | 'edits' | 'analytics' | 'tokens'>(
    'messages'
  )
...
const Sidebar = () => {
  ...
  useEffect(() => {
    const removeListener = window.api.fs.onFileChanged(
      (data: { event: string; path: string; timestamp?: number }) => {
    ...
    <div className="panel-base sidebar">
      ...
      <div className="sidebar-content">
        ...
        <div className="ch-conversations-section">
          ...
          >
            <span className="ch-section-icon">
              <ConversationIcon />
            </span>
            ...
            <div className="ch-conversations-content">
              ...
              <div className={`ch-conversation-list ${showAllConversations ? 'expanded' : ''}`}>
                {claudeSessions.length > 0 ? (
                  (showAllConversations ? claudeSessions : claudeSessions.slice(0, 6)).map(
                    (session) => (
                      <ConversationItem
                        key={session.id}
                        session={session}
              ...
              {claudeProject && claudeSessions.length > 6 && (
                <div className="ch-view-all">
                  <button onClick={() => setShowAllConversations(!showAllConversations)}>
                    {showAllConversations ? <>收起 ↑</> : <>查看全部 ({claudeSessions.length}) ↓</>}
                  </button>
                </div>
              )}
            </div>
                ...
                searchResults.map((result, index) => (
                  ...
                  >
                    <div className="search-result-info">
                      ...
                      <span className="search-result-location">:{result.line}</span>
                    </div>
                    <div className="search-result-preview">{result.content}</div>
                  </div>
                ))
              ) : (
                <div className="search-empty">未找到匹配项</div>
              )}
            </div>
          ) : (
      ...
      {/* Config Manager */}
      {showConfigManager && <ConfigManager onClose={() => setShowConfigManager(false)} />}
