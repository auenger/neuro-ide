          ...
          {isEditing ? (
            ...
            <>
              ...
              <div className="terminal-header-actions">
                <button className="terminal-action-btn" onClick={handleFit} title="调整尺寸">
                ...
                <button className="terminal-action-btn" onClick={handleStartEdit} title="重命名">
                  <svg viewBox="0 0 16 16" fill="currentColor">
...
const TerminalGrid = ({ sessionId }: TerminalGridProps) => {
  ...
  const [terminalOrder, setTerminalOrder] = useState<string[]>(terminals.map((t) => t.id))
    ...
    for (let rowIndex = 0; rowIndex < gridRows; rowIndex++) {
      ...
      rows.push(
        <Panel key={`row-${rowIndex}`} minSize={10} defaultSize={100 / gridRows}>
          <PanelGroup direction="horizontal">{cellsWithHandles}</PanelGroup>
        </Panel>
      )
  ...
  return (
    <div className="terminal-grid-container">
      ...
      >
        <SortableContext items={terminalOrder} strategy={rectSortingStrategy}>
          <PanelGroup direction="vertical">{renderRows()}</PanelGroup>
        </SortableContext>
