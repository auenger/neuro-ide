...
const MainPanel = () => {
  ...
  const handleRestartTerminal = async () => {
    if (activeTerminalId && activeSession) {
      window.dispatchEvent(
        new CustomEvent('terminal-restart', {
          detail: {
  ...
  const toggleMdCollapse = () => {
    ...
    if (newCollapsed) {
      // Collapsing: store current size then collapse
      const currentSize = mdPanelRef.current?.getSize()
      if (currentSize && currentSize > 8) {
        setMdPanelPrevSize(currentSize)
      }
      mdPanelRef.current?.collapse()
    } else {
      // Expanding: expand and restore previous size
      mdPanelRef.current?.expand()
  ...
  const currentPresetLabel =
    GRID_PRESETS.find((p) => p.rows === gridLayoutConfig.rows && p.cols === gridLayoutConfig.cols)
    ...
    <div className="panel-base main-panel">
      <PanelGroup direction="vertical">
        ...
        <Panel
          ...
          collapsedSize={5}
        >
          <MarkdownEditor isCollapsed={isMdCollapsed} onToggleCollapse={toggleMdCollapse} />
        </Panel>

        <PanelResizeHandle
          className={`resize-handle-horizontal ${isMdCollapsed ? 'disabled' : ''}`}
          disabled={isMdCollapsed}
        />
        ...
        <Panel defaultSize={82} minSize={20}>
          <div className="chat-console">
                        ...
                        {GRID_PRESETS.map((preset) => (
                          <button
                            ...
                            className={`grid-dropdown-item ${
                              ...
                              preset.cols === gridLayoutConfig.cols
                                ? 'active'
                                : ''
                            }`}
                            onClick={() => {
                ...
                <div className="tabs-list">
                  {activeSession.terminals.map((terminal) => (
                    <div
                      ...
                      onClick={() =>
                        useAppStore.getState().setActiveTerminal(activeSessionId, terminal.id)
                      }
                    >
                      ...
                      {activeSession.terminals.length > 1 && (
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            useAppStore
                              .getState()
                              .removeTerminalFromSession(activeSessionId, terminal.id)
                          }}
                          title="关闭终端"
                        >
                          <svg viewBox="0 0 16 16" fill="currentColor">
            ...
            {terminalLayoutMode === 'grid' && <TerminalGrid sessionId={activeSessionId} />}
          </div>
        </Panel>
      </PanelGroup>
    </div>
