    ...
    const refreshAffectedFolders = async () => {
      ...
      for (const [changedPath] of fileChanges) {
        ...
        for (const folder of expandedStarredFolders) {
          ...
          if (
            changedPath.startsWith(folder.path + '/') ||
            changedPath.startsWith(folder.path + '\\')
          ) {
        ...
        {starredItems
          ...
          .map((item) => {
            ...
            return (
              <div key={item.id}>
                <div
                  className={`starred-item ${dragOverItem === item.id ? 'drag-over' : ''} ${
                  ...
                  onClick={() =>
                    item.isDirectory ? toggleFolder(item.path, children) : handleFileClick(item)
                  }
                ...
                {item.isDirectory && isExpanded && children && (
                  <div className="starred-folder-content">{renderFileTree(children, 0)}</div>
                )}
              </div>
            )
          })}
