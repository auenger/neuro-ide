...
const ConfigManager = ({ onClose }: ConfigManagerProps) => {
    ...
    const loadDeletedSessions = async () => {
        ...
        try {
            ...
            const projectPaths = matchedProjects && matchedProjects.length > 0
                ? matchedProjects
                ...
                <div key={node.path} className="dir-tree-node">
                    <div className={`dir-tree-item ${ignored ? 'ignored' : ''}`}>
                        <span
                            ...
                            onClick={() => toggleDirNode(node, currentPath)}
                        >
                            {node.isExpanded ? (
                    ...
                    {node.isExpanded && node.children && node.children.length > 0 && (
                        <div className="dir-tree-children">
                            {renderDirTree(node.children, currentPath)}
                        </div>
                    )}
                </div>
            )
                                    ...
                                    <div className="editor-form">
                                        ...
                                        {!editForm.isBuiltIn && (
                                            <div className="form-actions">
                                                <button className="delete-btn" onClick={handleRoleDelete}>删除角色</button>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                            ...
                            <div className="config-section">
                                ...
                                {loadingDeleted ? (
                                    ...
                                    <div className="history-empty">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="48px" height="48px">
